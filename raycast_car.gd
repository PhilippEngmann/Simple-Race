...
func _physics_process(delta: float) -> void:
	...
	for wheel in wheels:
		...
		var shapecast: ShapeCast3D = wheel.get_child(1)
		...
		shapecast.add_exception(wheel.get_parent())
		...
		if not wheel.is_colliding(): continue # TODO: Remove
		if not shapecast.is_colliding(): continue
		
		var contact_point := wheel.get_collision_point() # TODO: Remove
		contact_point = shapecast.get_collision_point(0)
		...
		var suspension_force = (spring_force - damping_force) * shapecast.get_collision_normal(0)
